...
@section('content')
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div class="mb-8">
            ...
            <h1 class="text-3xl font-bold text-slate-900 dark:text-white">{{ __('Yangi Oraliq Nazorat yaratish') }}</h1>
        </div>
        ...
        <div
            ...
            <form action="{{ route('oraliq.store') }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-6">
                @csrf

                <div>
                    <label for="title" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{{ __('Sarlavha (Title)') }}</label>
                    <input type="text" name="title" id="title" required
                        ...
                        placeholder="{{ __('Masalan: 1-Oraliq Nazorat - Kompyuter tarmoqlari') }}">
                    @error('title') <p class="mt-1 text-sm text-red-500">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label for="description"
                        class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{{ __('Tavsif (Description - ixtiyoriy)') }}</label>
                    <textarea name="description" id="description" rows="4"
                        class="w-full px-4 py-3 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-2 focus:ring-cyan-500 transition-all"
                        placeholder="{{ __('Topshiriq shartlari va ko\'rsatmalar...') }}"></textarea>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="deadline"
                            class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{{ __('Muddat (Deadline)') }}</label>
                    ...
                    <div>
                        <label for="max_score"
                            class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{{ __('Maksimal ball (Max Score)') }}</label>
                        <input type="number" name="max_score" id="max_score" required min="1"
                            class="w-full px-4 py-3 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-2 focus:ring-cyan-500 transition-all"
                            placeholder="{{ __('Masalan: 20') }}">
                        <p class="mt-1 text-xs text-slate-400 italic">{{ __('Umumiy ball 50 dan oshmasligi kerak.') }} ({{ __('Hozirgi yig\'indi:') }} {{ $currentTotal }})</p>
                ...
                <div>
                    <label for="assignment_file"
                        class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{{ __('Topshiriq fayli (PDF, DOCX, PPTX)') }}</label>
                    <div
                        ...
                        <div class="space-y-1 text-center">
                            ...
                            <div class="flex text-sm text-slate-600 dark:text-slate-400">
                                <label for="assignment_file"
                                    ...
                                    <span>{{ __('Faylni yuklash') }}</span>
                                    <input id="assignment_file" name="assignment_file" type="file" class="sr-only" required
                                        accept=".pdf,.docx,.pptx">
                                </label>
                                <p class="pl-1">{{ __('yoki sudrab keling') }}</p>
                            </div>
                            <p class="text-xs text-slate-500">{{ __('PDF, DOCX, PPTX (Maks 50MB)') }}</p>
                        </div>
                    </div>
