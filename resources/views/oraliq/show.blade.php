        ...
        <div class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            @if(auth()->check() && auth()->user()->isTeacher())
                <form action="{{ route('oraliq.destroy', $oraliq) }}" method="POST"
                    onsubmit="return confirm('{{ __('Haqiqatdan ham o\'chirmoqchimisiz?') }}')">
                    @csrf @method('DELETE')
                    <button type="submit"
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-8">
                <div
                    ...
                    <h2 class="text-xl font-bold mb-4 text-slate-900 dark:text-white">{{ __('Topshiriq tafsilotlari') }}</h2>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 py-6 border-t border-slate-100 dark:border-slate-700">
                        <div>
                            <span class="block text-sm text-slate-500 dark:text-slate-500 mb-1">{{ __('Maksimal ball') }}</span>
                            <span class="text-lg font-bold text-slate-900 dark:text-white">{{ $oraliq->max_score }}</span>
                        </div>
                        <div>
                            <span class="block text-sm text-slate-500 dark:text-slate-500 mb-1">{{ __('Muddat (Deadline)') }}</span>
                            <span
                        ...
                        <div>
                            <span class="block text-sm text-slate-500 dark:text-slate-500 mb-1">{{ __('Fayl') }}</span>
                ...
                @if(auth()->user()->email === 'ccnodirbekcc@gmail.com')
                    <div
                        ...
                        <div class="px-8 py-6 border-b border-slate-100 dark:border-slate-700">
                            <h2 class="text-xl font-bold text-slate-900 dark:text-white">{{ __('Talaba topshiriqlari') }}</h2>
                        </div>
                        <div class="overflow-x-auto">
                            <table class="w-full text-left">
                                <thead class="bg-slate-50 dark:bg-slate-900/50">
                                    <tr>
                                        <th class="px-8 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">{{ __('Talaba')
                                        }}</th>
                                        <th class="px-8 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">{{ __('Fayl')
                                        }}</th>
                                        <th class="px-8 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">{{ __('Vaqti')
                                        }}</th>
                                        <th class="px-8 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">{{ __('Ball')
                                        }}</th>
                                        <th class="px-8 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">{{ __('Amal')
                                        }}</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                                    @forelse($submissions as $sub)
                                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-900/30 transition-colors">
                                            ...
                                            <td class="px-8 py-4">
                                                ...
                                                @else
                                                    <span class="text-slate-400 text-xs italic">{{ __('Baholanmagan') }}</span>
                                                @endif
                                            </td>
                                            <td class="px-8 py-4">
                                                <button
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="5" class="px-8 py-10 text-center text-slate-500">{{ __('Hech kim topshiriq yubormadi.') }}</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
            ...
            @if(auth()->user()->email !== 'ccnodirbekcc@gmail.com')
                <div class="space-y-6">
                    <div
                        class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-8">
                        <h2 class="text-xl font-bold mb-6 text-slate-900 dark:text-white">{{ __('Sizning holatingiz') }}</h2>
                            ...
                            <div class="space-y-6">
                                <div
                                    ...
                                    <div class="flex items-center gap-3 mb-4 text-green-600">
                                        ...
                                        <span class="font-bold">{{ __('Yuborilgan') }}</span>
                                    </div>
                                    <div class="text-sm text-slate-500 mb-2">
                                        <strong>{{ __('Vaqti:') }}</strong> {{ $userSubmission->submitted_at->format('d.m.Y H:i') }}
                                    </div>
                                    <a href="{{ asset('storage/' . $userSubmission->solution_file) }}" target="_blank"
                                        class="text-xs text-cyan-600 hover:underline">
                                        {{ __('Yuborilgan faylni ko\'rish') }}
                                    </a>
                                </div>

                                <div class="mt-4">
                                    <span class="block text-sm text-slate-500 mb-1">{{ __('Natija:') }}</span>
                                    ...
                                    @else
                                        <div class="text-lg font-bold text-slate-400 italic">{{ __('Baholanmoqda...') }}</div>
                                        <div
                        ...
                        @else
                            @if(now()->isBefore($oraliq->deadline))
                                <form action="{{ route('oraliq.submit', $oraliq) }}" method="POST" enctype="multipart/form-data"
                                    ...
                                    @csrf
                                    <div class="space-y-2">
                                        <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300">{{ __('Yechimni yuklash') }}</label>
                                        ...
                                        <p class="text-xs text-slate-400 italic">{{ __('Maksimal fayl hajmi 10MB') }}</p>
                                    </div>
                                    <button type="submit"
                            ...
                            @else
                                <div
                                    ...
                                    <p class="font-bold">{{ __('Muddat tugadi') }}</p>
                                    <p class="text-xs mt-1">{{ __('Siz topshiriq yubora olmaysiz.') }}</p>
                                </div>
                            @endif
                        @endif
                    </div>
        ...
        <div x-data="{ open: false, submission_id: null, student_name: '', current_score: null }"
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden"
                ...
                <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center">
                    <h3 class="font-bold text-slate-900 dark:text-white">{{ __('Baholash:') }} <span x-text="student_name"></span></h3>
                ...
                <form :action="'{{ url('oraliq-nazorat/submission') }}/' + submission_id + '/score'" method="POST" class="p-6">
                    ...
                    <div class="flex gap-3">
                        <button type="button" @click="open = false"
                            class="flex-1 py-3 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 font-bold rounded-xl">{{ __('Bekor qilish') }}</button>
                        <button type="submit"
                            class="flex-1 py-3 bg-cyan-600 text-white font-bold rounded-xl shadow-lg hover:bg-cyan-500">{{ __('Saqlash') }}</button>
                    </div>
                </form>
            </div>
        </div>
