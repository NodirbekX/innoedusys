...
@section('content')
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div class="mb-8">
            ...
            <h1 class="text-3xl font-bold text-slate-900 dark:text-white">{{ __('Oraliq Nazoratni tahrirlash') }}</h1>
        </div>
            ...
            <form action="{{ route('oraliq.update', $oraliq) }}" method="POST" enctype="multipart/form-data"
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4"
                        class="w-full px-4 py-3 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-2 focus:ring-cyan-500 transition-all"
                        placeholder="{{ __('Topshiriq shartlari va ko\'rsatmalar...') }}">{{ old('description', $oraliq->description) }}</textarea>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-xs text-slate-400 italic">{{ __('Boshqa topshiriqlar ballari bilan birga 50 dan oshmasligi kerak.') }} ({{ __('Hozirgi boshqalar:') }} {{ $currentTotal - $oraliq->max_score }})</p>
                ...
                <div>
                    <label for="assignment_file"
                        class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{{ __('Topshiriq fayli (Agar yangilamoqchi bo\'lsangiz yuklang)') }}</label>
                    <div
                        ...
                        <div class="space-y-1 text-center">
                            ...
                            <p class="text-xs text-slate-500">{{ __('PDF, DOCX, PPTX (Maks 10MB)') }}</p>
                            <p class="text-xs text-cyan-600 font-bold mt-2">{{ __('Joriy fayl:') }}
