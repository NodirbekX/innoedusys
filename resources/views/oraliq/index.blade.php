...
@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div class="flex justify-between items-center mb-8">
            <div>
                <h1 class="text-3xl font-bold text-slate-900 dark:text-white">{{ __('Oraliq Nazorat') }}</h1>
                <p class="mt-2 text-slate-600 dark:text-slate-400">{{ __('Barcha oraliq nazorat topshiriqlari ro\'yxati') }}</p>
            </div>
        ...
        <div
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                <div>
                    <h2 class="text-xl font-bold text-slate-900 dark:text-white">{{ __('Umumiy natija') }}</h2>
                    <p class="text-sm text-slate-500 dark:text-slate-400">{{ __('Barcha topshiriqlar bo\'yicha umumiy ball') }}</p>
                </div>
            ...
            @if(auth()->check() && auth()->user()->isTeacher())
                <div class="mt-4 p-3 bg-cyan-50 dark:bg-cyan-900/20 rounded-lg border border-cyan-100 dark:border-cyan-800">
                    <p class="text-xs text-cyan-700 dark:text-cyan-400">
                        <strong>{{ __('Admin ma\'lumoti:') }}</strong> {{ __('Topshiriqlar maksimal ballari yig\'indisi:') }}
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div class="p-6">
                        <div class="flex justify-between items-start mb-4">
                            ...
                            <span
                                ...
                                {{ $assignment->deadline->format('d.m.Y H:i') }} {{ __('gacha') }}
                            </span>
                        </div>
                                ...
                                @if($sub)
                                    @if($sub->score !== null)
                                        <div class="inline-flex items-center gap-2 px-3 py-1 bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 text-xs font-bold rounded-lg border border-green-200 dark:border-green-800">
                                            ...
                                            {{ __('BAHOLANDI:') }} {{ $sub->score }} / {{ $assignment->max_score }}
                                        </div>
                                    @else
                        ...
                        <p class="text-slate-600 dark:text-slate-400 text-sm mb-4 line-clamp-2">
                            {{ $assignment->description ?? __('Tavsif berilmagan') }}
                        </p>

                        <div class="flex items-center justify-between mt-6">
                            <div class="text-sm text-slate-500 dark:text-slate-400">
                                <strong>{{ __('Maks. ball:') }}</strong> {{ $assignment->max_score }}
                            </div>
            ...
            @empty
                <div class="col-span-full py-20 text-center">
                    ...
                    <p class="text-slate-500 dark:text-slate-400">{{ __('Hozircha oraliq nazoratlar yo\'q.') }}</p>
                </div>
            @endforelse
        </div>
    </div>
