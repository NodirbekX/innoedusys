    ...
    <div class="flex min-h-screen
        ...
        <aside class="w-72 p-5 hidden md:block
            ...
            <h2 class="text-xl font-bold mb-6 tracking-wide
                       text-cyan-600 dark:text-cyan-400">{{ __('🌐 Tarmoq Mavzulari') }}</h2>
            ...
            @else
                <div class="text-center py-8">
                    <p class="text-slate-500 dark:text-slate-400">{{ __('Mavzular mavjud emas') }}</p>
        ...
        <main class="flex-1 p-8">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-3xl font-extrabold tracking-wide fade-in
                           text-cyan-600 dark:text-cyan-300">{{ __('📝 Topshiriqlarni Boshqarish') }}</h2>
            ...
            @if($materials->count() == 0)
                <div class="text-center py-12 rounded-xl fade-in
                    ...
                    <p class="text-lg mb-4 text-slate-600 dark:text-slate-400">{{ __('Mavzular mavjud emas.') }}</p>
                    <a href="{{ route('home.create') }}"
                       class="inline-block font-semibold px-6 py-2 rounded-lg transition
                              bg-cyan-500 text-white hover:bg-cyan-600
                              dark:bg-cyan-500 dark:text-slate-900 dark:hover:bg-cyan-400">
                        {{ __('+ Birinchi Mavzuni Yaratish') }}
                    </a>
                </div>
            @else
                @if(session('success'))
                ...
                @if($assignments->count() > 0)
                    <div class="mb-4 p-3 rounded-lg fade-in
                                ...
                                dark:bg-slate-800 dark:border-cyan-500/20">
                        <p class="text-sm text-slate-500 dark:text-slate-400">{{ __('Topshiriqlar boshqarilmoqda:') }}</p>
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                                            <p class="text-slate-500 dark:text-slate-500">
                                                {{ __('📚 Mavzu:') }} <span class="text-slate-700 dark:text-slate-300">{{ $assignment->material->title }}</span>
                                            </p>
                                            <p class="text-slate-500 dark:text-slate-500">
                                                {{ __('⏰ Muddati:') }} <span class="text-slate-700 dark:text-slate-300">{{ \Carbon\Carbon::parse($assignment->deadline)->format('d M Y, H:i') }}</span>
                                            </p>
                                            <p class="text-slate-500 dark:text-slate-500">
                                                {{ __('🎯 Maksimal Ball:') }} <span class="text-slate-700 dark:text-slate-300">{{ $assignment->max_score }}</span>
                                            </p>
                                            <p class="text-slate-500 dark:text-slate-500">
                                                {{ __('📊 Topshirilganlar:') }} <span class="text-slate-700 dark:text-slate-300">{{ $assignment->submissions()->count() }}</span>
                                            </p>
                                        </div>
                                        ...
                                        @if($assignment->file_path)
                                            <a href="{{ asset('storage/' . $assignment->file_path) }}"
                                               ...
                                               class="inline-block mt-3 transition
                                                ...
                                                {{ __('📄 Topshiriq Faylini Yuklab Olish') }}
                                            </a>
                                        @endif
                                    </div>
                                    ...
                                    <div class="flex flex-col gap-2 ml-4">
                                        <a href="{{ route('teacher.assignments.submissions', $assignment->id) }}"
                                           class="font-semibold px-4 py-2 rounded-lg transition text-center
                                            ...
                                            {{ __("Topshirilganlarni Ko'rish") }} ({{ $assignment->submissions()->count() }})
                                        </a>
                                        ...
                                        <form action="{{ route('teacher.assignments.destroy', $assignment->id) }}"
                                              method="POST"
                                              onsubmit="return confirm(@json(__('Bu topshiriqni o\'chirishni xohlaysizmi?')));">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                ...
                @else
                    ...
                    <div class="text-center py-12 rounded-xl fade-in
                                bg-slate-50 border border-slate-200
                                dark:bg-slate-800 dark:border-cyan-500/20">
                        <p class="text-lg mb-4 text-slate-600 dark:text-slate-400">{{ __('Bu mavzu uchun topshiriqlar topilmadi.') }}</p>
                        <button onclick="createAssignment()"
                                class="font-semibold px-6 py-2 rounded-lg transition
                                       bg-cyan-500 text-white hover:bg-cyan-600
                                       dark:bg-cyan-500 dark:text-slate-900 dark:hover:bg-cyan-400">
                            {{ __('Birinchi Topshiriqni Yaratish') }}
                        </button>
                    </div>
                @endif
            @endif
