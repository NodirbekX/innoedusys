    ...
    <div class="max-w-6xl mx-auto p-6">
        <div class="mb-6">
            <a href="{{ route('teacher.assignments.index', ['material_id' => $assignment->material_id]) }}"
               class="inline-block mb-4 transition
                ...
                {{ __('← Topshiriqlarga Qaytish') }}
            </a>
            <h2 class="text-3xl font-extrabold tracking-wide mb-2
                       text-cyan-600 dark:text-cyan-300">
                {{ __('📥 Topshirilganlar –') }} {{ $assignment->title }}
            </h2>
            <div class="text-sm text-slate-600 dark:text-slate-400">
                <p>{{ __('📚 Mavzu:') }} <span class="text-slate-700 dark:text-slate-300">{{ $assignment->material->title }}</span></p>
                <p>{{ __('⏰ Muddati:') }} <span class="text-slate-700 dark:text-slate-300">{{ \Carbon\Carbon::parse($assignment->deadline)->format('d M Y, H:i') }}</span></p>
                <p>{{ __('🎯 Maksimal Ball:') }} <span class="text-slate-700 dark:text-slate-300">{{ $assignment->max_score }}</span></p>
            </div>
        </div>
                    ...
                    <div class="scan relative rounded-xl p-6 overflow-hidden transition-all duration-300
                        ...
                        <div class="flex justify-between items-start">
                            <div class="flex-1">
                                ...
                                <div class="mb-3">
                                    <p class="text-sm mb-1 text-slate-500 dark:text-slate-500">{{ __('Topshirilgan vaqti:') }}</p>
                                ...
                                <a href="{{ asset('storage/' . $submission->file_path) }}"
                                   ...
                                   class="inline-flex items-center gap-2 transition mb-4
                                          ...
                                          dark:text-cyan-400 dark:hover:text-cyan-300">
                                    <span>📄</span>
                                    <span>{{ __('Topshirilganni Yuklab Olish') }}</span>
                                ...
                                @if($submission->isGraded())
                                    <div class="mt-3 p-3 rounded-lg
                                        ...
                                        <p class="text-sm text-green-700 dark:text-green-300">
                                            {{ __('✅ Baholandi:') }} <span class="font-bold">{{ $submission->score }} / {{ $assignment->max_score }}</span>
                                        </p>
                                        <p class="text-xs mt-1 text-slate-500 dark:text-slate-400">
                                            {{ __('Baholangan vaqti:') }} {{ \Carbon\Carbon::parse($submission->updated_at)->format('d M Y, H:i') }}
                                        </p>
                                    </div>
                                @else
                                    <div class="mt-3 p-3 rounded-lg
                                        ...
                                        <p class="text-sm text-yellow-700 dark:text-yellow-300">{{ __('⏳ Baholash Kutilmoqda') }}</p>
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="ml-6">
                                <form action="{{ route('teacher.assignments.score', $submission->id) }}"
                                    ...
                                    <label class="block text-sm font-medium mb-2
                                                  text-slate-700 dark:text-cyan-300">{{ __('Ball') }}</label>
                                    ...
                                    <button type="submit"
                                        ...
                                        {{ $submission->isGraded() ? __('Ballni Yangilash') : __('Baholash') }}
                                    </button>
                                </form>
                            </div>
                        </div>
        ...
        @else
            <div class="text-center py-12 rounded-xl
                ...
                <p class="text-lg text-slate-600 dark:text-slate-400">{{ __('Hali hech kim topshirmagan.') }}</p>
            </div>
        @endif
    </div>
@endsection
