...
@section('content')
    <div class="max-w-4xl mx-auto p-6">
        <h2 class="text-3xl font-extrabold mb-6 tracking-wide
                   text-cyan-600 dark:text-cyan-300">{{ __('✏️ Topshiriqni Tahrirlash') }}</h2>
        ...
        <form method="POST" action="{{ route('teacher.assignments.update', $assignment->id) }}" enctype="multipart/form-data"
            ...
            <div>
                <label class="block text-sm font-medium mb-2
                              text-slate-700 dark:text-cyan-300">{{ __('Mavzu *') }}</label>
            ...
            <div>
                <label class="block text-sm font-medium mb-2
                              text-slate-700 dark:text-cyan-300">{{ __('Sarlavha *') }}</label>
                <input type="text" name="title" value="{{ old('title', $assignment->title) }}" required
                       placeholder="{{ __('Topshiriq sarlavhasi') }}"
                       class="w-full p-3 rounded-lg border focus:outline-none
                              bg-slate-50 border-slate-300 text-slate-900 focus:border-cyan-500
                              dark:bg-slate-900 dark:text-slate-100 dark:border-cyan-500/20 dark:focus:border-cyan-400">
            </div>
            ...
            <div>
                <label class="block text-sm font-medium mb-2
                              text-slate-700 dark:text-cyan-300">{{ __('Tavsif') }}</label>
                <textarea name="description" rows="4"
                          placeholder="{{ __('Topshiriq tavsifi') }}"
                          class="w-full p-3 rounded-lg border focus:outline-none
                                 bg-slate-50 border-slate-300 text-slate-900 focus:border-cyan-500
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium mb-2
                                  text-slate-700 dark:text-cyan-300">{{ __('Muddati *') }}</label>
                ...
                <div>
                    <label class="block text-sm font-medium mb-2
                                  text-slate-700 dark:text-cyan-300">{{ __('Maksimal Ball *') }}</label>
            ...
            @if($assignment->file_path)
                <div class="p-3 rounded-lg
                    ...
                    <p class="text-sm mb-2 text-slate-500 dark:text-slate-400">{{ __('Joriy fayl:') }}</p>
            ...
            <div>
                <label class="block text-sm font-medium mb-2
                    ...
                    {{ $assignment->file_path ? __('Topshiriq Faylini Almashtirish') : __('Topshiriq Fayli (Ixtiyoriy)') }}
                </label>
                ...
                <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">{{ __('Qabul qilinadigan formatlar: PDF, DOC, DOCX, TXT (Maks: 10MB)') }}</p>
            </div>
