        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div
                    ...
                    <div class="mb-4 md:mb-0">
                        <p class="text-sm text-gray-600">{{ __('Type:') }} <span
                                class="font-bold uppercase">{{ $test->type }}</span></p>
                        <p class="text-sm text-gray-600">{{ __('Time Limit:') }} <span class="font-bold">{{ $test->time_limit }}
                                {{ __('mins') }}</span></p>
                        <p class="text-sm text-gray-600">{{ __('Schedule:') }} {{ $test->start_at->format('M d H:i') }} -
                    ...
                    <div>
                        <form action="{{ route('admin.tests.publish', $test) }}" method="POST" class="inline">
                            ...
                            <button type="submit"
                                ...
                                {{ $test->published ? __('Unpublish') : __('Publish Test') }}
                            </button>
                        </form>
                        ...
                        <form action="{{ route('admin.tests.destroy', $test) }}" method="POST" class="inline ml-2"
                            onsubmit="return confirm('{{ __('Are you sure? This will delete all results associated with this test!') }}');">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6 bg-white border-b border-gray-200">
                    <h3 class="font-bold text-lg mb-4">{{ __('Add New Question') }}</h3>
                    <form action="{{ route('admin.tests.questions.store', $test) }}" method="POST">
                        @csrf
                        <div class="mb-4">
                            <label class="block text-gray-700 text-sm font-bold mb-2">{{ __('Question Text') }}</label>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="mb-2">
                                <label class="block text-gray-700 text-sm font-bold mb-1">{{ __('Option A') }}</label>
                                <input type="text" name="option_a"
                                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                    required>
                            </div>
                            <div class="mb-2">
                                <label class="block text-gray-700 text-sm font-bold mb-1">{{ __('Option B') }}</label>
                                <input type="text" name="option_b"
                                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                    required>
                            </div>
                            <div class="mb-2">
                                <label class="block text-gray-700 text-sm font-bold mb-1">{{ __('Option C') }}</label>
                                <input type="text" name="option_c"
                                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                    required>
                            </div>
                            <div class="mb-2">
                                <label class="block text-gray-700 text-sm font-bold mb-1">{{ __('Option D') }}</label>
                        ...
                        <div class="mb-4 mt-4">
                            <label class="block text-gray-700 text-sm font-bold mb-2">{{ __('Correct Option') }}</label>
                            <select name="correct_option"
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                required>
                                <option value="a">{{ __('Option A') }}</option>
                                <option value="b">{{ __('Option B') }}</option>
                                <option value="c">{{ __('Option C') }}</option>
                                <option value="d">{{ __('Option D') }}</option>
                            </select>
                        </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <h3 class="font-bold text-lg mb-4">{{ __('Questions') }} ({{ $test->questions->count() }})</h3>
                        ...
                        <div class="mb-6 p-4 border rounded bg-gray-50">
                            <div class="flex justify-between">
                                ...
                                <form action="{{ route('admin.tests.questions.destroy', $question) }}" method="POST"
                                    onsubmit="return confirm('{{ __('Delete question?') }}');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                        class="text-red-500 hover:text-red-700 text-sm font-bold">{{ __('Delete') }}</button>
                                </form>
                            </div>
                    ...
                    @empty
                        <p class="text-gray-500 italic">{{ __('No questions added yet.') }}</p>
                    @endforelse
                </div>
            </div>
