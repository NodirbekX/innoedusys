...
<head>
    ...
    <title>{{ __("Ro'yxatdan o'tish") }} - InnoEduSys</title>
    ...
    <div class="w-full max-w-md rounded-2xl p-8 shadow-2xl animate-[fadeInUp_0.8s_ease-out]
        ...
        <div class="text-center mb-8 animate-[float_3s_ease-in-out_infinite]">
            ...
            <h1 class="text-3xl font-bold tracking-tight animate-[fadeInUp_0.6s_ease-out_0.2s_both]
                       text-slate-800 dark:text-white">{{ __('InnoEduSys') }}</h1>
            <p class="text-sm animate-[fadeInUp_0.6s_ease-out_0.4s_both]
                      text-slate-600 dark:text-indigo-300/80">{{ __('Yangi akkaunt yaratish') }}</p>
        </div>
        ...
        <form method="POST" action="{{ route('register') }}">
            ...
            <!-- Name -->
            <div class="mb-5 animate-[fadeInUp_0.6s_ease-out_0.1s_both]">
                <label for="name" class="block text-sm font-medium mb-2
                                         text-slate-700 dark:text-slate-300">{{ __('Ism') }}</label>
                <div class="relative">
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name') }}" required autofocus autocomplete="name" placeholder="{{ __('Ismingizni kiriting') }}"
                ...
                @if ($errors->get('name'))
                    <div class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $errors->first('name') }}</div>
                @endif
            </div>

            <!-- Email Address -->
            <div class="mb-5 animate-[fadeInUp_0.6s_ease-out_0.2s_both]">
                <label for="email" class="block text-sm font-medium mb-2
                                          text-slate-700 dark:text-slate-300">{{ __('Elektron pochta') }}</label>
                <div class="relative">
                    <span class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none text-indigo-500 dark:text-indigo-400">
                    ...
                    <input type="email" id="email" name="email" value="{{ old('email') }}" required autocomplete="username" placeholder="{{ __('your@email.com') }}"
                           class="w-full pl-10 pr-4 py-3 rounded-lg border transition-all duration-300 outline-none
                                  bg-white/50 border-slate-300 text-slate-900 placeholder-slate-400
                                  hover:bg-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20
                                  dark:bg-slate-900/50 dark:border-indigo-500/30 dark:text-white dark:placeholder-slate-500
            ...
            <!-- Password -->
            <div class="mb-5 animate-[fadeInUp_0.6s_ease-out_0.3s_both]">
                <label for="password" class="block text-sm font-medium mb-2
                                             text-slate-700 dark:text-slate-300">{{ __('Parol') }}</label>
                <div class="relative">
                    <span class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none text-indigo-500 dark:text-indigo-400">
                    ...
                    <input type="password" id="password" name="password" required autocomplete="new-password" placeholder="{{ __('Kamida 8 ta belgi') }}"
                           class="w-full pl-10 pr-4 py-3 rounded-lg border transition-all duration-300 outline-none
                                  bg-white/50 border-slate-300 text-slate-900 placeholder-slate-400
                                  hover:bg-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20
                                  dark:bg-slate-900/50 dark:border-indigo-500/30 dark:text-white dark:placeholder-slate-500
                ...
                @if ($errors->get('password'))
                    <div class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $errors->first('password') }}</div>
                @endif
            </div>
            ...
            <div class="mb-6 animate-[fadeInUp_0.6s_ease-out_0.4s_both]">
                <label for="password_confirmation" class="block text-sm font-medium mb-2
                                                          text-slate-700 dark:text-slate-300">{{ __('Parolni tasdiqlash') }}</label>
                <div class="relative">
                    <span class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none text-indigo-500 dark:text-indigo-400">
                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                    </span>
                    <input type="password" id="password_confirmation" name="password_confirmation" required autocomplete="new-password" placeholder="{{ __('Parolni qayta kiriting') }}"
                           class="w-full pl-10 pr-4 py-3 rounded-lg border transition-all duration-300 outline-none
                                  bg-white/50 border-slate-300 text-slate-900 placeholder-slate-400
                                  hover:bg-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20
                                  dark:bg-slate-900/50 dark:border-indigo-500/30 dark:text-white dark:placeholder-slate-500
                ...
                @if ($errors->get('password_confirmation'))
                    <div class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $errors->first('password_confirmation') }}</div>
                @endif
            </div>

            <div class="flex flex-col sm:flex-row justify-between items-center gap-4 mt-6">
                <a href="{{ route('login') }}" class="text-sm font-medium transition
                                                      text-indigo-600 hover:text-indigo-500
                                                      dark:text-indigo-400 dark:hover:text-indigo-300">{{ __("Allaqachon ro'yxatdan o'tganmisiz?") }}</a>
                <button type="submit" class="w-full sm:w-auto py-3 px-6 rounded-lg text-white font-bold text-base transition-all duration-300 shadow-lg
                                             bg-gradient-to-r from-indigo-500 to-purple-600
                                             hover:scale-105 hover:shadow-indigo-500/50
                                             active:scale-95">{{ __("Ro'yxatdan o'tish") }}</button>
            </div>
        </form>
        ...
        <div class="text-center mt-8 pt-6 border-t
            ...
            <div class="flex items-center justify-center gap-2 text-xs text-slate-500 dark:text-slate-400">
                ...
                <span>&copy; {{ date('Y') }} {{ __("InnoEduSys — Xavfsiz va zamonaviy ta'lim tizimi") }}</span>
            </div>
        </div>
    </div>
</div>
