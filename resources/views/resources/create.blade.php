...
@section('content')
    ...
    <div class="rounded-xl p-8 max-w-lg mx-auto shadow-2xl fade-in
        ...
        <h1 class="text-3xl font-extrabold mb-6 tracking-wide
                   text-cyan-600 dark:text-cyan-300">{{ __('Manba Qo\'shish') }}</h1>
        ...
        <form action="{{ route('resources.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                <label for="title" class="block text-sm font-medium mb-2
                                          text-slate-700 dark:text-slate-300">{{ __('Kitob Nomi:') }}</label>
                <input type="text"
                       name="title"
                       id="title"
                       value="{{ old('title') }}"
            ...
            <div>
                <label for="file" class="block text-sm font-medium mb-2
                                         text-slate-700 dark:text-slate-300">{{ __('Kitob Fayli:') }}</label>
                <input type="file"
                       name="file"
                       id="file"
                       required
