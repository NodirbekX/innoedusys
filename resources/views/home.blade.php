    ...
    <div class="flex min-h-screen page-enter
        ...
        <main class="flex-1 p-8 w-96">
            ...
            @if(isset($selectedMaterial))
                    ...
                    <div id="scoreModal" class="hidden fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center"
                        ...
                        <div class="rounded-2xl p-8 max-w-md w-full mx-4 shadow-2xl text-center
                            ...
                            <h3 class="text-3xl font-bold mb-4
                                                   text-cyan-700 dark:text-cyan-300">{{ __('🎉 Video Tugadi!') }}</h3>
                            <div class="mb-6">
                ...
                @if($selectedMaterial->presentation_path)
                    ...
                    <a href="{{ asset('storage/' . $selectedMaterial->presentation_path) }}" target="_blank"
                        ...
                        <div>
                            ...
                            <p class="text-sm text-slate-500 dark:text-slate-400"> {{ __('Prezentatsiyasi') }}</p>
                        </div>
                    </a>
                @endif
                ...
                {{-- ===== GLOSSARY ===== --}}
                <div class="mb-10">
                    <h3 class="text-2xl font-bold tracking-wide mb-4 text-cyan-700 dark:text-cyan-300">{{ __('📖 glossary') }}</h3>
                    ...
                    <div class="space-y-4">
                                ...
                                <div class="flex justify-between items-start">
                                    ...
                                    @if(auth()->check() && auth()->user()->isTeacher())
                                        <div class="flex gap-2 ml-4 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                                            ...
                                            <form action="{{ route('glossary.destroy', $glossary->id) }}" method="POST" onsubmit="return confirm('{{ __('O\'chirishni tasdiqlaysizmi?') }}')" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="text-sm font-semibold text-red-600 hover:text-red-500 bg-red-50 dark:bg-red-900/30 px-2 py-1 rounded">{{ __('Delete') }}</button>
                                            </form>
                                        </div>
                                    @endif
                                </div>
                        ...
                        @if($selectedMaterial->glossaries->isEmpty())
                            <p class="text-slate-500 italic text-center p-4">{{ __('Ushbu mavzuga oid lug\'at topilmadi.') }}</p>
                        @endif
                    </div>
                    ...
                    <div class="mt-8 p-6 rounded-xl border-2 border-dashed border-slate-300 dark:border-cyan-500/30 bg-slate-50 dark:bg-slate-800/50">
                            <h4 class="font-bold mb-4 text-lg text-slate-700 dark:text-cyan-400 flex items-center gap-2">
                                <span>➕</span> {{ __('Yangi so\'z qo\'shish') }}
                            </h4>
                            <form action="{{ route('glossary.store', $selectedMaterial->id) }}" method="POST" class="grid gap-4">
                                @csrf
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div>
                                        <label class="block text-xs font-bold uppercase text-slate-500 mb-1">{{ __('Termin') }}</label>
                                    ...
                                    <div class="md:col-span-2">
                                        <label class="block text-xs font-bold uppercase text-slate-500 mb-1">{{ __('Izoh') }}</label>
                        ...
                        <div id="editGlossaryModal" class="hidden fixed inset-0 bg-black/80 z-[9999] flex items-center justify-center backdrop-blur-sm">
                             <div class="bg-white dark:bg-slate-800 p-8 rounded-2xl w-full max-w-lg shadow-2xl border border-slate-200 dark:border-cyan-500/20 transform transition-all scale-100">
                                 <h3 class="text-2xl font-bold mb-6 text-slate-800 dark:text-white border-b pb-4 dark:border-slate-700">{{ __('Tahrirlash') }}</h3>
                                 <form id="editGlossaryForm" method="POST" class="space-y-4">
                                     @csrf @method('PUT')
                                     <div>
                                         <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{{ __('Termin') }}</label>
                                     ...
                                     <div>
                                         <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{{ __('Izoh') }}</label>
                                     ...
                                     <div class="flex justify-end gap-3 pt-4">
                                         <button type="button" onclick="document.getElementById('editGlossaryModal').classList.add('hidden')" class="px-4 py-2 text-slate-600 hover:text-slate-800 dark:text-slate-400 dark:hover:text-white font-semibold">{{ __('Bekor qilish') }}</button>
                                         <button type="submit" class="bg-cyan-600 text-white px-6 py-2 rounded-lg font-bold hover:bg-cyan-500 shadow-lg">{{ __('Saqlash') }}</button>
                                     </div>
                                 </form>
                             </div>
                        </div>
                ...
                @if(auth()->check() && auth()->user()->isTeacher())
                    <div class="mt-6 flex gap-3 fade-in">
                        <a href="{{ route('home.edit', $selectedMaterial->id) }}" class="px-4 py-2 rounded-lg transition
                            ...
                            {{ __('Mavzuni Tahrirlash') }}
                        </a>
                        <form action="{{ route('home.destroy', $selectedMaterial->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="submit" onclick="return confirm('{{ __('Bu mavzuni o\'chirishni xohlaysizmi?') }}')" class="px-4 py-2 rounded-lg transition
                                                       bg-red-500 text-white hover:bg-red-400">
                                {{ __('Mavzuni O\'chirish') }}
                            </button>
                        </form>
                    </div>
                @endif
            ...
            (function () {
                ...
                video.addEventListener('seeked', function () {
                    ...
                    if (video.currentTime > maxWatchedTime + 2) {
                        ...
                        alert("{{ __('Oldinga o\'tib bo\'lmaydi. Videoni tartib bilan tomosha qiling.') }}");
                    }
                });
                ...
                video.addEventListener('ended', function () {
                    ...
                    if (answeredQuestions.size === sortedTests.length) {
                        ...
                        if (foundUnanswered) {
                            alert("{{ __('Videoni yakunlash uchun barcha savollarga javob bering.') }}");
                        }
                    }
                });
                ...
                submitAnswerBtn.addEventListener('click', async function () {
                    if (!selectedAnswer || currentQuestionIndex === null) {
                        alert("{{ __('Iltimos, javobni tanlang.') }}");
                        return;
                    }
                    ...
                    try {
                        ...
                        if (data.success) {
                            ...
                            if (data.is_correct) {
                                answerFeedback.innerHTML = `
                                        <div class="p-4 rounded-lg
                                            ...
                                            ✓ {{ __('To\'g\'ri! Siz') }} ${data.earned_points} {{ __('ball oldingiz.') }}
                                        </div>
                                    `;
                            } else {
                                answerFeedback.innerHTML = `
                                        <div class="p-4 rounded-lg
                                            ...
                                            ✗ {{ __('Noto\'g\'ri. To\'g\'ri javob:') }} ${data.correct_answer}. {{ __('Siz 0 ball oldingiz.') }}
                                        </div>
                                    `;
                            }
                            ...
                            alert("{{ __('Javobni yuborishda xatolik yuz berdi. Iltimos, qayta urinib ko\'ring.') }}");
                        }
                    } catch (error) {
                    ...
                    try {
                        ...
                        if (data.success) {
                            ...
                            document.getElementById('scoreDetails').textContent =
                                `${data.earned_points} / ${data.total_points} {{ __('ball') }}`;
                ...
                questionModal.addEventListener('click', function (e) {
                    if (e.target === questionModal) {
                        ...
                        alert("{{ __('Davom etish uchun savolga javob bering.') }}");
                    }
                });
            })();
        </script>
