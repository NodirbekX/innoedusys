...
@section('content')
    ...
    <div class="max-w-4xl mx-auto page-enter">
        ...
        <div class="rounded-2xl p-8
            ...
            <form action="{{ route('home.store') }}" method="POST" enctype="multipart/form-data" id="upload-form">
                ...
                {{-- Sarlavha (Title) --}}
                <div class="mb-5">
                    <label class="block mb-2 text-slate-700 dark:text-slate-300">{{ __('Mavzu Sarlavhasi') }}</label>
                ...
                {{-- Video --}}
                <div class="mb-5">
                    <label class="block mb-2 text-slate-700 dark:text-slate-300">{{ __('Video (ixtiyoriy)') }}</label>
                    ...
                    <div id="progress-container" class="mt-3 hidden">
                        ...
                        <p id="progress-text" class="text-sm mt-1
                                                    text-cyan-600 dark:text-cyan-300">{{ __('0%') }}</p>
                    </div>
                </div>

                {{-- Taqdimot (Presentation) --}}
                <div class="mb-5">
                    <label class="block mb-2 text-slate-700 dark:text-slate-300">{{ __('Taqdimot (ixtiyoriy)') }}</label>
                ...
                {{-- Tavsif (Description) --}}
                <div class="mb-6">
                    <label class="block mb-2 text-slate-700 dark:text-slate-300">{{ __('Tavsif (ixtiyoriy)') }}</label>
    ...
    <script>
        ...
        <div class="test-card mb-5 rounded-xl p-5
            ...
            <label class="block mb-2 text-slate-700 dark:text-slate-300">{{ __('Savol') }}</label>
            <input type="text"
            ...
            <label class="block mb-2 text-slate-700 dark:text-slate-300">{{ __('Variantlar (har bir qatorga bittadan)') }}</label>
            ...
            <label class="block mb-2 text-slate-700 dark:text-slate-300">{{ __('Toʻgʻri Javob') }}</label>
            <input type="text"
            ...
            <label class="block mb-2 text-slate-700 dark:text-slate-300">{{ __('Koʻrsatish Vaqti (sekundda)') }}</label>
            <input type="number"
                   name="tests[${testIndex}][time]"
                   min="1" required
                   placeholder="{{ __('masalan, 120') }}"
                   class="w-full p-2 rounded-lg border mb-3
                          bg-white border-slate-300 text-slate-900
                          dark:bg-slate-900 dark:border-cyan-500/20 dark:text-slate-100">

            <label class="block mb-2 text-slate-700 dark:text-slate-300">{{ __('Ball (Points)') }}</label>
            <input type="number"
                   name="tests[${testIndex}][points]"
                   min="1" required
                   value="1"
                   placeholder="{{ __('masalan, 5') }}"
    ...
    <script>
        document.getElementById('upload-form').addEventListener('submit', function(e) {
            ...
            const msgUploading = @json(__('Yuklanmo...'));
            const msgSaved = @json(__('Mavzuni Saqlash'));
            const msgPct = @json(__('0%'));
            const msgErrPrefix = @json(__('Xatolik: '));
            const msgUnknown = @json(__("Noma'lum xatolik"));
            const msgValidationIntro = @json(__('Validatsiya xatoliklari:'));
            const msgServerErr = @json(__('Server xatoligi'));
            const msgHttpErr = @json(__('Xatolik yuz berdi. Status: '));
            const msgNetwork = @json(__("Tarmoq xatoligi. Iltimos, qayta urinib ko'ring."));

            // Disable submit button
            submitBtn.disabled = true;
            submitBtn.textContent = msgUploading;
                ...
                if (xhr.status === 200) {
                    try {
                        ...
                        if (response.success) {
                            ...
                            alert(msgErrPrefix + (response.message || msgUnknown));
                            resetForm();
                        }
                    } catch (e) {
                    ...
                    try {
                        const response = JSON.parse(xhr.responseText);
                        if (response.errors) {
                            let errorMsg = msgValidationIntro + '\n';
                            for (let field in response.errors) {
                                errorMsg += response.errors[field].join('\n') + '\n';
                            }
                            alert(errorMsg);
                        } else {
                            alert(msgErrPrefix + (response.message || msgServerErr));
                        }
                    } catch (e) {
                        alert(msgHttpErr + xhr.status);
                    }
                    resetForm();
                }
            });
            ...
            function resetForm() {
                ...
                submitBtn.textContent = msgSaved;
                submitBtn.classList.remove('opacity-50', 'cursor-not-allowed');
                progressContainer.classList.add('hidden');
                progressBar.style.width = '0%';
                progressText.textContent = msgPct;
            }
