...
@section('content')
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div class="flex items-center justify-between mb-10">
            <div>
                <h1 class="text-3xl font-black text-slate-900 dark:text-white">{{ __('Yakuniy Test') }}</h1>
                <p class="text-slate-500 font-medium">{{ __('Iltimos, barcha savollarga diqqat bilan javob bering.') }}</p>
            </div>
            <div class="px-6 py-3 bg-slate-900 text-white rounded-2xl font-black shadow-lg">
                <span id="countdown" class="font-mono text-xl">
                    @if($settings)
                        {{ $settings->end_time->format('H:i') }} {{ __('gacha') }}
                    @endif
                </span>
            </div>
        </div>
        ...
        <form action="{{ route('yakuniy.submit') }}" method="POST" id="testForm">
            ...
            <div class="mt-12">
                <button type="submit"
                    ...
                    onclick="return confirm('{{ __('Testni yakunlamoqchimisiz? Javoblarni keyin o\'zgartirib bo\'lmaydi.') }}')">
                    {{ __('TESTNI YAKUNLASH') }}
                </button>
            </div>
        </form>
