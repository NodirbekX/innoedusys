...
@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div class="flex justify-between items-center mb-10">
            <div>
                <h1 class="text-4xl font-black text-slate-900 dark:text-white tracking-tight">{{ __('Yakuniy Nazorat') }}</h1>
                <p class="mt-2 text-slate-600 dark:text-slate-400">{{ __('Final imtihon topshirish moduli') }}</p>
            </div>
            @if(auth()->check() && auth()->user()->isTeacher())
                <div class="flex gap-3">
                    <a href="{{ route('yakuniy.settings') }}"
                        class="px-5 py-2.5 bg-slate-800 text-white rounded-xl font-bold hover:bg-slate-700 transition">{{ __('vaqtni belgilash') }}</a>
                    <a href="{{ route('yakuniy.questions') }}"
                        class="px-5 py-2.5 bg-cyan-600 text-white rounded-xl font-bold hover:bg-cyan-500 transition">{{ __('Savollar qo\'shish')
                        }}</a>
                    <a href="{{ route('yakuniy.results') }}"
                        class="px-5 py-2.5 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-500 transition">{{ __('Natijalar') }}</a>
                </div>
            @endif
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-8">
                <div
                    ...
                    <div class="p-8">
                        <h2 class="text-2xl font-bold text-slate-900 dark:text-white mb-6">{{ __('Imtihon holati') }}</h2>
                        ...
                        @if(!$isEligible && !auth()->user()->isAdmin())
                            <div class="flex flex-col items-center justify-center py-12 text-center">
                                ...
                                <h3 class="text-2xl font-black text-slate-900 dark:text-white mb-2">{{ __('Ruxsat berilmadi') }}</h3>
                                <p class="text-slate-600 dark:text-slate-400 max-w-md mx-auto">
                                    {{ __('Yakuniy topshirish uchun yetarli ball to\'planmadi. Kamida 30 ball oraliq nazoratlardan to\'plangan bo\'lishi kerak.') }}
                                </p>
                                <div
                                    ...
                                    {{ __('Sizning ballingiz:') }} {{ $oraliqScore }} / 50
                                </div>
                            </div>
                        @elseif($myResult)
                            <div class="flex flex-col items-center justify-center py-12 text-center">
                                ...
                                <h3 class="text-2xl font-black text-slate-900 dark:text-white mb-2">{{ __('Imtihon yakunlangan') }}</h3>
                                <p class="text-slate-600 dark:text-slate-400 mb-6 font-medium">{{ __('Siz o\'z natijangizni qo\'lga kiritdingiz') }}</p>
                                <div class="text-5xl font-black text-cyan-600 dark:text-cyan-400 mb-2">
                                    {{ $myResult->total_score }}
                                </div>
                                <div class="text-sm font-bold text-slate-400 uppercase tracking-widest">{{ __('Yakuniy Natija') }}</div>
                                <div class="mt-8 text-xs text-slate-500">{{ __('Topshirilgan vaqt:') }}
                        ...
                        @elseif($status === 'not_set')
                            <div class="py-12 text-center">
                                ...
                                <p class="text-xl font-bold text-slate-500 dark:text-slate-400">{{ __('Yakuniy nazorat hali belgilanmagan') }}</p>
                            </div>
                        @elseif($status === 'not_started')
                            <div class="py-12 text-center">
                                ...
                                <h3 class="text-2xl font-black text-slate-900 dark:text-white mb-2">{{ __('Hali boshlanmadi') }}</h3>
                                <p class="text-slate-600 dark:text-slate-400">{{ __('Yakuniy nazorat belgilangan vaqtda ochiladi.') }}</p>
                        ...
                        @elseif($status === 'ended')
                            <div class="py-12 text-center">
                                ...
                                <h3 class="text-2xl font-black text-slate-900 dark:text-white mb-2">{{ __('Vaqt tugagan') }}</h3>
                                <p class="text-slate-600 dark:text-slate-400 font-medium">{{ __('Yakuniy nazorat vaqti yakunlandi.') }}</p>
                                <div class="mt-4 text-sm text-slate-400 lowercase italic">
                                    {{ __('Tugash vaqti:') }} {{ $settings->end_time->format('d.m.Y H:i') }}
                                </div>
                            </div>
                        @else
                                ...
                                <div
                                    ...
                                    <ul class="space-y-3 text-sm text-cyan-800 dark:text-cyan-300">
                                        <li class="flex items-start gap-2">
                                            ...
                                            {{ __('Sizda faqat bitta urinish mavjud.') }}
                                        </li>
                                        <li class="flex items-start gap-2">
                                            <span class="mt-1 flex-shrink-0 w-1.5 h-1.5 bg-cyan-500 rounded-full"></span>
                                            {{ __('Testni yakunlagandan so\'ng javoblarni o\'zgartirib bo\'lmaydi.') }}
                                        </li>
                                        <li class="flex items-start gap-2">
                                            <span class="mt-1 flex-shrink-0 w-1.5 h-1.5 bg-cyan-500 rounded-full"></span>
                                            {{ __('Belgilangan vaqt ichida topshirish shart.') }}
                                        </li>
                                    </ul>
                                </div>
            ...
            <div class="space-y-8">
                <div
                    ...
                    <h3 class="text-white font-bold text-xl mb-6">{{ __('Ma\'lumotlar') }}</h3>

                    <div class="space-y-6">
                        <div>
                            <div class="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1">{{ __('Oraliq ball') }}</div>
                        ...
                        @if($settings)
                            <div>
                                <div class="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1">{{ __('Imtihon oralig\'i') }}</div>
                                <div class="text-slate-200 font-medium">
                                    {{ $settings->start_time->format('d.m.Y H:i') }} {{ __('dan') }}<br>
                                    {{ $settings->end_time->format('d.m.Y H:i') }} {{ __('gacha') }}
                                </div>
                            </div>
                        @endif
                    </div>
