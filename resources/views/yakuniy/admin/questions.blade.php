    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div class="flex justify-between items-center mb-10">
            <div class="flex items-center gap-4">
                ...
                <h1 class="text-3xl font-black text-slate-900 dark:text-white">{{ __('Savollar boshqaruvi') }}</h1>
            </div>
        ...
        <div class="space-y-6">
            @forelse($questions as $index => $q)
                <div class="bg-white dark:bg-slate-800 rounded-3xl p-8 shadow-xl border border-slate-200 dark:border-slate-700">
                    <div class="flex justify-between items-start mb-6">
                        <div>
                            <span class="text-slate-400 font-bold uppercase tracking-widest text-xs">{{ __('Savol') }}
                                #{{ $index + 1 }}</span>
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <form action="{{ route('yakuniy.questions.destroy', $q) }}" method="POST"
                                onsubmit="return confirm('{{ __('O\'chirmoqchimisiz?') }}')">
                                @csrf
                                @method('DELETE')
            ...
            @empty
                <div
                    ...
                    <p class="text-slate-500 font-bold">{{ __('Hozircha savollar yo\'q.') }}</p>
                </div>
            @endforelse
        </div>
        ...
        <div id="addQuestionModal"
            ...
            <div
                ...
                <div
                    ...
                    <h2 class="text-2xl font-black text-slate-900 dark:text-white">{{ __('Yangi savol qo\'shish') }}</h2>
                ...
                <form action="{{ route('yakuniy.questions.store') }}" method="POST"
                    ...
                    @csrf
                    <div>
                        <label class="block text-sm font-black text-slate-700 dark:text-slate-300 uppercase mb-2">{{ __('Savol matni') }}</label>
                    ...
                    <div>
                        <label
                            class="block text-sm font-black text-slate-700 dark:text-slate-300 uppercase mb-2">{{ __('Ball') }}</label>
                    ...
                    <div class="space-y-4">
                        <label class="block text-sm font-black text-slate-700 dark:text-slate-300 uppercase mb-2">{{ __('Javob variantlari') }}</label>

                        @for($i = 0; $i < 4; $i++)
                            <div class="flex gap-4 items-center">
                                <input type="radio" name="correct_option" value="{{ $i }}" {{ $i == 0 ? 'checked' : '' }}
                                    class="w-6 h-6 text-cyan-600 focus:ring-cyan-500">
                                <input type="text" name="options[]" required placeholder="{{ __('Variant') }} {{ $i + 1 }}"
                        ...
                        @endfor
                        <p class="text-xs text-slate-400 italic font-medium mt-2">{{ __('Dugmacha (radio) orqali to\'g\'ri javobni belgilang.') }}</p>
                    </div>
