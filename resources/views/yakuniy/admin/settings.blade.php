...
@section('content')
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div class="mb-10 flex items-center gap-4">
            ...
            <h1 class="text-3xl font-black text-slate-900 dark:text-white">{{ __('Imtihon vaqtini sozlash') }}</h1>
        </div>
        ...
        <div
            ...
            <form action="{{ route('yakuniy.settings.store') }}" method="POST" class="p-8 space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                        <label
                            class="block text-sm font-black text-slate-700 dark:text-slate-300 uppercase tracking-widest mb-3">{{ __('Imtihon boshlanishi') }}</label>
                    ...
                    <div>
                        <label
                            class="block text-sm font-black text-slate-700 dark:text-slate-300 uppercase tracking-widest mb-3">{{ __('Imtihon tugashi') }}</label>
                ...
                <div class="p-6 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-800">
                    <div class="flex items-start gap-4">
                        ...
                        <div>
                            <h4 class="font-bold text-slate-900 dark:text-white">{{ __('Eslatma') }}</h4>
                            <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">
                                {{ __('Ushbu vaqt oralig\'ida barcha ruxsat etilgan talabalar test topshirishi mumkin bo\'ladi.') }}
                            </p>
                        </div>
                    </div>
                </div>
